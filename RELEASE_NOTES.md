...
* Added vcfvalidate, a standalone VCFv4.5 data line validator built on vcf-std: reports all spec violations with line numbers and section references (exit 1 on any error, including empty/header-only files); all regression test outputs are now validated with it as part of the test suite
...
* vcfputtogetheragain gained --norm-multiallelic, which mirrors `bcftools norm -m-`: all ALT alleles of multi-allelic input records are kept in the merged record (instead of the zig semantics that drop them), so merged records always satisfy the VCF spec and MULTI=ALTPROBLEM only marks true genotype conflicts - on the scaffold612 sample this fixes 256 previously invalid records
